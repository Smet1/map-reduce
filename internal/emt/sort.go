...
// for sorting.
type Chunk func(*bufio.Reader) ([]byte, error)
...
func (s *sorted) flush() error {
	...
	file, err := os.OpenFile(filepath.Join(s.tmpDir, strconv.Itoa(s.numFiles))+".csv", os.O_CREATE|os.O_WRONLY, 0o644)
	if err != nil {
		return err
	}
...
func (s *sorted) Close() error {
	...
	for i := 0; i < s.numFiles; i++ {
		file, err := os.OpenFile(filepath.Join(s.tmpDir, strconv.Itoa(i))+".csv", os.O_RDONLY, 0)
		if err != nil {
	...
	if s.numFiles == 1 {
		...
		if copyErr != nil {
			return errors.Wrap(copyErr, "unable to copy sorted data to output")
		}
	} else {
...
func (s *sorted) finalSort(files map[int]*bufio.Reader) error {
	...
	for {
		...
		if writeErr != nil {
			return errors.Wrap(writeErr, "error writing to final output")
		}
		...
		if file != nil {
			...
			if err != nil {
				return errors.Wrap(err, "error replacing entry on heap")
			}
